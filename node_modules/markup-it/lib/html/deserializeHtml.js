...
var _require3 = require('../'),
    Deserializer = _require3.Deserializer;

var _require4 = require('../'),
    BLOCKS = _require4.BLOCKS,
    INLINES = _require4.INLINES,
    MARKS = _require4.MARKS,
    CONTAINERS = _require4.CONTAINERS,
    VOID = _require4.VOID,
    LEAFS = _require4.LEAFS,
    Block = _require4.Block,
    Inline = _require4.Inline,
    Text = _require4.Text,
    Mark = _require4.Mark;
...
var deserialize = Deserializer().then(function (state) {
    var nodes = parse(state.text);

    // Normalize the document, since for now HTML introduces a lot of
    // unwanted empty text nodes.
    var normalizedNodes = Slate.State.create({
        document: Slate.Document.create({ nodes: nodes })
    }, {
        normalize: true
    }).document.nodes;

    return state.push(normalizedNodes).skip(state.text.length);
});
...
function parse(str) {
    ...
    var parser = new htmlparser.Parser({
        onopentag: function onopentag(tagName, attribs) {
            if (BLOCK_TAGS[tagName]) {
                ...
                var textNode = Text.createFromString('\n', marks);
                appendNode(textNode);
            }
            // else ignore
        ...
        ontext: function ontext(text) {
            ...
            var textNode = Text.createFromString(cleanText, marks);
            appendNode(textNode);
        },
    ...
    var rootNodes = stack.peek().nodes;
